...
static void
_metadatas_closure_free(gpointer data)
{
...
static gboolean
_emit_metadatas_results_idle(gpointer data)
{
  struct _metadatas_common_closure *mcc;
...
static void
_emit_metadatas_results(struct _metadatas_common_closure *mcc)
{
  /* Emit results */
  g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
                  (GSourceFunc)_emit_metadatas_results_idle, mcc,
                  (GDestroyNotify)_metadatas_closure_free);
}

static void
_get_metadata_tracker_cb(GHashTable *result,
                         GError *error,
                         gpointer user_data)
{
  ...
  /* Save result */
  if (result)
    g_hash_table_insert(mc->common->metadatas, mc->object_id, result);
...
static void
_get_metadatas_tracker_cb(GList *results,
                          GError *error,
                          gpointer user_data)
{
  struct _metadatas_closure *mc = (struct _metadatas_closure *)user_data;
...
static gboolean
_calculate_duration_is_needed(GHashTable *metadata,
                              gchar **metadata_keys,
                              const gchar *object_id)
{
  ...
  if (util_is_duration_requested((const gchar **)metadata_keys))
  {
    ...
    if (category == CATEGORY_MUSIC_PLAYLISTS)
    {
      ...
      {
        /* Single playlist. */
        calculate = util_calculate_playlist_duration_is_needed(metadata);
        g_free(pls_uri);
      }
      else
      {
                          ...
                          const GError *error)
{
  ...
  if (!error && metadata)
  {
    ...
    if (gval)
    {
      ...
      gval = mafw_metadata_first(pls_mc->metadata_value,
...
static void
_get_metadata_tracker_from_playlist_cb(GHashTable *result,
                                       GError *error,
                                       gpointer user_data)
{
  if (!error && result)
...
static void
_get_metadatas_tracker_from_playlist_cb(GList *results,
                                        GError *error,
                                        gpointer user_data)
{
  GList *current_obj;
  ...
  while (current_obj && current_result)
  {
                                      ...
                                      current_obj->data))
    {
      ...
      /* If there aren't more playlists (remaining == 0), this
         ...
         in this case. */
      mafw_tracker_source_get_playlist_duration(nmc->common->source,
                                                nmc->object_id,
                                                _add_playlist_duration_cb,
                                                nmc);
    }
...
static void
_get_metadata_tracker_from_music_cb(GHashTable *result,
                                    GError *error,
                                    gpointer user_data)
{
  GValue *gval;
  ...
  if (result)
  {
    ...
    gval = mafw_metadata_first(result, MAFW_METADATA_KEY_CHILDCOUNT_1);
...
static void
_get_metadata_tracker_from_root_videos_cb(GHashTable *result,
                                          GError *error,
                                          gpointer user_data)
{
  GValue *gvalcur;
  ...
  if (result)
  {
    ...
    {
      gvalcur = mafw_metadata_first(result, MAFW_METADATA_KEY_DURATION);
      gvalnew = mafw_metadata_first(mc->metadata_value,
                                    MAFW_METADATA_KEY_DURATION);
  ...
  if (result)
  {
    ...
    gvalcur = mafw_metadata_first(result, MAFW_METADATA_KEY_CHILDCOUNT_1);
...
static void
_get_metadata_tracker_from_root_music_cb(GHashTable *result,
                                         GError *error,
                                         gpointer user_data)
{
  struct _metadatas_closure *mc = (struct _metadatas_closure *)user_data;
...
static void
_get_metadata_cb(MafwSource *source,
                 GHashTable *metadatas,
                 gpointer user_data,
                 const GError *error)
{
...
static gboolean
_update_metadata_idle(gpointer data)
{
                                  ...
                                  gpointer user_data)
{
  ...
  mcc->metadatas = g_hash_table_new_full(g_str_hash,
                                         g_str_equal,
  ...
  while (object_ids[i])
  {
    category = util_extract_category_info(object_ids[i], &genre, &artist,
                                          &album, &clip);
    ...
    else if (clip)
    {
      ...
      switch (category)
      {
        case CATEGORY_VIDEO:
        {
          if (!video_mc)
          {
            video_mc = g_new0(struct _metadatas_closure, 1);
            video_mc->common = mcc;
          }

          video_mc->object_ids = g_list_prepend(video_mc->object_ids,
                                                g_strdup(object_ids[i]));
          video_clips = g_list_prepend(video_clips, clip);
          break;
        }
        case CATEGORY_MUSIC_PLAYLISTS:
        {
          if (!playlist_mc)
          {
            playlist_mc = g_new0(struct _metadatas_closure, 1);
             ...
             * calculated */
            playlist_metadata_keys = g_strdupv(mcc->metadata_keys);
          }

          playlist_mc->object_ids = g_list_prepend(playlist_mc->object_ids,
                                                   g_strdup(object_ids[i]));
          playlist_clips = g_list_prepend(playlist_clips, clip);
        ...
        default:
        {
          if (!audio_mc)
          {
            audio_mc = g_new0(struct _metadatas_closure, 1);
            audio_mc->common = mcc;
          }

          audio_mc->object_ids = g_list_prepend(audio_mc->object_ids,
                                                g_strdup(object_ids[i]));
          audio_clips = g_list_prepend(audio_clips, clip);
          break;
        }
      }
    }
    else
    {
      ...
      switch (category)
      {
        case CATEGORY_ROOT:
        {
          ti_get_metadata_from_music(mcc->metadata_keys,
                                     ROOT_TITLE,
        ...
        case CATEGORY_MUSIC_PLAYLISTS:
        {
          ti_get_metadata_from_playlists(mcc->metadata_keys,
  ...
  if (playlist_mc)
  {
    ...
    ti_get_metadata_from_playlist(clips,
                                 ...
                                 gpointer user_data)
{
  ...
  if (!object_id)
  {
    ...
    cb(self, object_id, (const gchar **)failed_keys, user_data, error);
  ...
  {
    _update_metadata_data = g_new0(struct _update_metadata_closure, 1);
