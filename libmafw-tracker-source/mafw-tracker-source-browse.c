...
static gboolean
_add_playlist_duration_idle(gpointer data);

/*
 * Provides a new browse id
 */
static gint
_get_next_browse_id(MafwTrackerSource *self)
{
...
static gboolean
_emit_browse_results_idle(gpointer data)
{
...
static inline void
_register_pending_browse_operation(MafwTrackerSource *source,
                                   struct _browse_closure *bc)
{
...
static inline void
_remove_pending_browse_operation(MafwTrackerSource *source,
                                 struct _browse_closure *bc)
{
  source->priv->pending_browse_ops =
...
static void
_browse_closure_free(gpointer data)
{
  struct _browse_closure *bc;
  GList *iter;
...
static void
_emit_browse_results(struct _browse_closure *bc)
{
  gint remaining;
  ...
  g_idle_add_full(G_PRIORITY_DEFAULT_IDLE,
                  (GSourceFunc)_emit_browse_results_idle, bc,
                  (GDestroyNotify)_browse_closure_free);
}

static void
_emit_browse_error(struct _browse_closure *bc,
                   GError *error)
{
  GError *mafw_error;
...
static void
_add_object_id_prefix_to_list(gchar *object_id_prefix,
                              GList *list,
                              gboolean escape)
{
  GList *iter;
  ...
  while (iter != NULL)
  {
    ...
    if (iter->data && !g_str_has_prefix((gchar *)iter->data,
                                        MAFW_URI_SOURCE_UUID "::"))
    {
      if (escape)
        data = mafw_tracker_source_escape_string(iter->data);
      else
        data = g_strdup(iter->data);

      g_free(iter->data);
      iter->data = g_strconcat(object_id_prefix, "/", data, NULL);
      g_free(data);
    }
...
static void
_browse_tracker_cb(MafwResult *clips, GError *error, gpointer user_data)
{
  ...
  if (error == NULL)
  {
    ...
    _add_object_id_prefix_to_list(bc->object_id_prefix, clips->ids, TRUE);
...
static void
_add_playlist_duration_cb(MafwSource *self,
                          guint browse_id,
                          ...
                          const GError *error)
{
  ...
  struct _browse_closure *playlists_bc = (struct _browse_closure *)user_data;
  ...
  if (!error && metadata)
  {
    ...
    if (gval)
    {
      mafw_metadata_add_int(playlists_bc->current_metadata_value->data,
  ...
  playlists_bc->current_metadata_value = g_list_next(
...
_add_playlist_duration_idle(gpointer data)
{
  ...
  pls_id = (playlists_bc->current_id) ? playlists_bc->current_id->data : NULL;
  ...
  {
    mafw_tracker_source_get_playlist_duration(playlists_bc->source, pls_id,
                                              _add_playlist_duration_cb, data);
...
static void
_browse_playlists_tracker_cb(MafwResult *clips,
                             GError *error,
                             gpointer user_data)
{
  struct _browse_closure *playlists_bc = (struct _browse_closure *)user_data;
  ...
  if (!error)
  {
    ...
    _add_object_id_prefix_to_list(playlists_bc->object_id_prefix, clips->ids,
                                  TRUE);

    /* Add the results to the browse closure */
    playlists_bc->ids = g_list_concat(playlists_bc->ids, clips->ids);
    ...
    if (util_is_duration_requested((const gchar **)playlists_bc->metadata_keys))
    {
      /* Maybe we have to recalculate some of them.
         Set the initial playlist to process in an idle. */
      playlists_bc->current_id = playlists_bc->ids;
      playlists_bc->current_metadata_value = playlists_bc->metadata_values;
...
static void
_consolidate_metadata_cb(MafwSource *source,
                         const gchar *object_id,
                         GHashTable *metadata,
                         gpointer user_data,
                         const GError *error)
{
  ...
  if (!error)
  {
    bc->ids = g_list_prepend(bc->ids, g_strdup(object_id));
    g_hash_table_ref(metadata);
    bc->metadata_values = g_list_prepend(bc->metadata_values, metadata);
  }
...
static gchar *
_build_object_id(const gchar *item1, ...)
{
  va_list args;
  gchar *next_item;
  gchar *escaped;
...
static void
_browse_enqueue_videos_cb(MafwResult *clips,
                          GError *error,
                          gpointer user_data)
{
  struct _browse_closure *bc = (struct _browse_closure *)user_data;
...
static GHashTable *
_new_metadata_from_untracked_resource(gchar *uri,
                                      gchar **keys)
{
  GHashTable *metadata;
  gint i = 0;
  ...
  for (i = 0; keys[i] != NULL; i++)
  {
    if (!strcmp(keys[i], MAFW_METADATA_KEY_TITLE))
    {
      ...
      mafw_metadata_add_str(metadata, keys[i], unescaped_title);
    ...
    else if (!strcmp(keys[i], MAFW_METADATA_KEY_URI))
    {
      ...
      if (uri[0] == '/')
        fixed_uri = g_filename_from_uri(uri, NULL, NULL);
      else
...
static void
_construct_playlist_entries_result(struct _browse_closure *bc,
                                   ...
                                   MafwResult *clips)
{
  ...
  while (iter != NULL)
  {
    ...
    if (g_str_has_prefix(uri, "file://"))
    {
      ...
      escape_pathname = mafw_tracker_source_escape_string(pathname);
      objectid = g_strconcat(bc->object_id_prefix, "/", escape_pathname, NULL);
      g_free(pathname);
    ...
    if (objectid && (tracker_metadatas != NULL) &&
        (metadata = g_hash_table_lookup(tracker_metadatas, objectid)))
    {
      ...
      clips->metadata_values = g_list_prepend(clips->metadata_values,
                                              metadata);
    }
    else
    {
      ...
      metadata = _new_metadata_from_untracked_resource(uri, bc->metadata_keys);

      clips->ids = g_list_prepend(clips->ids, clip);
      clips->metadata_values = g_list_prepend(clips->metadata_values, metadata);
    }
                   ...
                   gpointer user_data)
{
  ...
  if ((bc->index >= bc->offset) && (bc->index < bc->offset + bc->count))
  {
    ...
    if (unescaped_uri[0] == '/')
    {
      escaped_uri = g_filename_to_uri(unescaped_uri, NULL, NULL);
    }
    else
    {
      escaped_uri = g_uri_escape_string(unescaped_uri,
    ...
    /* Check if the URI is local (tracker can resolve the metadata) or not  */
    if (g_str_has_prefix(escaped_uri, "file://"))
    {
      filename = g_filename_from_uri(escaped_uri, NULL, NULL);

      if (filename)
        bc->pls_local_ids = g_list_prepend(bc->pls_local_ids, filename);
    }
...
static void
_browse_playlist_tracker_cb(MafwSource *self,
                            GHashTable *tracker_metadatas,
                            gpointer user_data,
                            const GError *error)
{
...
static gboolean
_get_playlist_entries(const gchar *pls_uri,
                      struct _browse_closure *bc,
                      GError **error)
{
  TotemPlParser *pl;
  ...
  else
  {
    if (bc->pls_local_ids != NULL)
    {
      ...
      /* Reverse the list */
      bc->pls_local_ids = g_list_reverse(bc->pls_local_ids);
      ...
      mafw_tracker_source_get_metadatas(
        bc->source, (const gchar **)local_objectids,
        (const gchar *const *)bc->metadata_keys, _browse_playlist_tracker_cb,
        bc);
    ...
    else if (bc->pls_uris != NULL)
    {
      ...
      /* We do not have local references, but we have
         ...
         tracker result */
      _browse_playlist_tracker_cb(bc->source, NULL, bc, NULL);
    }
    else
    {
...
static gboolean
_send_error_idle(gpointer data)
{
  struct _browse_error_closure *bec = data;

  bec->callback(bec->source, MAFW_SOURCE_INVALID_BROWSE_ID, 0, 0, NULL, NULL,
                bec->user_data, bec->error);
...
static void
_browse_songs_branch(const gchar *genre,
                     const gchar *artist,
                     const gchar *album,
                     struct _browse_closure *bc)
{
...
static void
_browse_albums_branch(const gchar *album,
                      struct _browse_closure *bc)
{
  gchar *escaped_album;
...
static void
_browse_artists_branch(const gchar *artist,
                       const gchar *album,
                       struct _browse_closure *bc)
{
  if (bc->recursive)
...
static void
_browse_genres_branch(const gchar *genre,
                      const gchar *artist,
                      const gchar *album,
                      struct _browse_closure *bc)
{
...
static void
_browse_root(struct _browse_closure *bc)
{
  gchar *object_id;
...
static void
_browse_videos_branch(struct _browse_closure *bc)
{
  /* Browsing /videos */
  bc->object_id_prefix = _build_object_id(TRACKER_SOURCE_VIDEOS,
                                          NULL);
...
static void
_browse_music_branch(struct _browse_closure *bc)
{
...
static gboolean
_browse_playlists_branch(const gchar *playlist,
                         struct _browse_closure *bc)
{
  GError *error = NULL;
                                  ...
                                  GError **error)
{
  ...
  pending_browse_ops = MAFW_TRACKER_SOURCE(self)->priv->pending_browse_ops;

  for (iter = pending_browse_ops; iter != NULL; iter = g_list_next(iter))
  {
...
static void
_get_playlist_duration_cb(MafwSource *self,
                          guint browse_id,
                          gint remaining_count,
                          guint index,
                          const gchar *object_id,
                          ...
                          const GError *error)
{
  ...
  if (!error && metadata)
  {
    ...
    if (gval)
      duration_bc->pls_duration += g_value_get_int(gval);
  }
  ...
  if (remaining_count == 0)
  {
    ...
    if (pls_uri)
    {
      ...
      ti_set_playlist_duration(pls_uri, duration_bc->pls_duration);
      g_free(pls_uri);
    }
    ...
    if (duration_bc->pls_duration > 0)
    {
      ...
      mafw_metadata_add_int(duration_metadata,
                            MAFW_METADATA_KEY_DURATION,
...
void
mafw_tracker_source_get_playlist_duration(MafwSource *self,
                                          const gchar *object_id,
                                          MafwSourceBrowseResultCb callback,
                                          gpointer user_data)
{
  ...
  struct _browse_closure *pls_duration_bc = g_new0(struct _browse_closure, 1);
